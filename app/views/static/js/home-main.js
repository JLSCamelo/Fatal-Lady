document.addEventListener('DOMContentLoaded', function() {
  ...
  if (hero && heroContent && heroScrollIndicator && heroOverlay) { // <-- Verifique heroOverlay
    ...
    const overlayOpacity = baseOpacity + (scrollPercent * (targetOpacity - baseOpacity));

    // Atualiza o background do overlay dinamicamente
    heroOverlay.style.background = `linear-gradient(135deg, rgba(0,0,0,${overlayOpacity}) 0%, rgba(70, 7, 7, ${overlayOpacity + 0.06}) 100%)`;
  }
  ...
  const parallaxElements = document.querySelectorAll('.story-image img, .category-image img');
  window.addEventListener('scroll', function() {
